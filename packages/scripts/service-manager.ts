...
const services: ServiceConfig[] = [
  ...
  },
  {
    ...
    dependencies: ['supabase'], // This dependency will cause supabase to start twice since it's also started directly in startAll()
    requiredServices: ['docker'],
  },
];
...
class ServiceManager {
  ...
  private startedServices: Set<string> = new Set(); // Track started services
  ...
  private async startService(service: ServiceConfig): Promise<void> {
    // Skip if service is already started
    if (this.startedServices.has(service.name)) {
      return;
    }
    ...
    try {
      ...
      this.startedServices.add(service.name);
  ...
  private async stopService(service: ServiceConfig): Promise<void> {
    ...
    try {
      ...
      await this.executeCommand({ command, args });
      this.startedServices.delete(service.name);
